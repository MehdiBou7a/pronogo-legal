---
layout: default
title: Terms of Service — PronoGo
description: The rules for using the PronoGo application.
---

# Terms of Service — PronoGo

**Last updated**: May 19, 2026
**Version**: 1.0

[← Back to home](../) · [Version française](../terms_fr/)

---

## 1. Introduction

These Terms of Service ("ToS") govern your use of the **PronoGo** mobile application ("the Application"), published by **Mehdi Bouhaouala** ("we").

By downloading, installing, or using the Application, you accept these ToS without reservation. If you do not accept these terms, you must not use PronoGo.

---

## 2. Service description

PronoGo is a **mobile sports prediction game based exclusively on virtual currencies** (tokens, pronocoins). The service allows you to:

- Make predictions on European football matches.
- Collect and evolve virtual cards.
- Compose weekly teams (deck).
- Participate in rankings (weekly / monthly / season).
- Join private leagues with friends or public leagues.
- Complete missions and challenges (Tribunal).
- Progress through an XP and level system.

### ⚠️ No real-money gambling

**PronoGo is neither a sports betting platform, nor an online casino, nor a gambling service.** All currencies in the Application (tokens, pronocoins, packs, etc.) are **virtual**, with no real monetary value, **non-exchangeable** for money, goods, or services outside the Application. Any in-app purchases only allow you to acquire virtual currencies or gameplay items — they are never investments, deposits, or bets entitling you to monetary winnings.

PronoGo is not regulated as a gambling service.

---

## 3. Eligibility

The use of the Application is reserved for persons who:

- Are **at least 13 years old** (16 years in the European Economic Area, unless parental consent in accordance with applicable national law).
- Are **legally capable** of contracting under their national law.
- **Reside in a country where PronoGo is distributed** by Apple App Store and Google Play. PronoGo is notably not distributed in Belgium.

If you are a minor, you must obtain prior consent from your legal representative before using the Application.

---

## 4. Account creation and management

### 4.1 Registration

Registration requires a valid email address and a strong password. You undertake to provide accurate information and keep it up to date.

### 4.2 Nickname

You can choose a nickname (display name) publicly visible in rankings and the friend system. The following are prohibited:

- Insulting, racist, sexist, homophobic, or otherwise offensive nicknames.
- Nicknames impersonating a third party (public figure, other player).
- Nicknames containing personal contact information (phone, address).
- Nicknames containing advertising messages or URLs.

We reserve the right to modify or delete any non-compliant nickname, without notice.

### 4.3 Account security

You are responsible for maintaining the confidentiality of your password and any activity performed on your account. Notify us immediately at `support@fcpronox.com` in case of unauthorized access.

### 4.4 One account per person

Only one account is allowed per natural person. The creation of multiple accounts ("multi-accounting") is strictly prohibited and may result in suspension or deletion of all affected accounts.

---

## 5. Acceptable use

### 5.1 User commitments

By using PronoGo, you undertake not to:

- Use **bots**, scripts, exploits, or automated techniques.
- **Modify, decompile, or reverse-engineer** the Application.
- **Sell, buy, or exchange** accounts, cards, virtual currencies for real money or outside the official channels of the Application.
- **Manipulate rankings** through collusion, exploitation of bugs, or fraudulent behavior.
- **Harass** other users (via nicknames, private leagues, friend system).
- Use the Application for **illegal purposes**.

### 5.2 Sanctions

In case of violation, we reserve the right, without notice or compensation, to:

- Issue a warning.
- Temporarily or permanently suspend your account.
- Delete your account and all associated data.
- Initiate legal proceedings if necessary.

---

## 6. Virtual currencies, packs, and IAP (in-app purchases)

### 6.1 Nature of currencies

The virtual currencies of PronoGo (tokens, pronocoins):

- Have **no monetary value** outside the Application.
- **Cannot be exchanged** for money, goods, or services.
- **Cannot be transferred** to other users (outside of provided gameplay mechanics).
- **Are consumable** and may expire (notably upon account deletion).

### 6.2 In-app purchases (post-V1)

Where applicable, the Application may offer in-app purchases allowing acquisition of virtual currencies or gameplay items (packs, boosters, etc.). These purchases:

- Are final and **non-refundable** (unless applicable law requires otherwise, or in case of proven malfunction on our part).
- Entitle you only to the purchased virtual items.
- Should not be considered as investments or stakes.

Transactions are managed by Apple App Store or Google Play depending on your platform.

### 6.3 Account deletion and losses

Upon account deletion, **all your virtual currencies, cards, packs, and progression are permanently lost** without financial compensation. It is your responsibility to verify this before clicking "Delete my account".

---

## 7. Intellectual property

### 7.1 Our content

All elements of PronoGo (source code, design, brand, logos, text, illustrations, gameplay mechanics, database) are the property of us or our partners (notably football-data.org for match data), protected by copyright, trademark law, and database rights.

You benefit from a personal, non-exclusive, non-transferable, and limited license to use the Application in compliance with these ToS.

### 7.2 Logos and club names

The names and logos of football clubs displayed in the Application are the property of their respective owners. Their display is for sporting information purposes and does not imply any affiliation, sponsorship, or endorsement.

---

## 8. Service availability

We strive to maintain the service available 24/7 but cannot guarantee absolute availability. We may temporarily interrupt the service for maintenance, updates, or in case of force majeure, without incurring liability.

### 8.1 Service evolution

We reserve the right, at any time, to:

- Modify, add, or remove features.
- Adjust gameplay rules (scoring, probabilities, rates).
- Modify the virtual economy (prices, currencies).
- Terminate the service.

You will be informed of major changes by in-app notification or email.

---

## 9. Liability

### 9.1 Limitation

To the extent permitted by applicable law:

- We cannot be held liable for **indirect damages** (loss of chance, loss of enjoyment, reputational harm).
- Our total liability, all damages combined, shall not exceed the amount you have paid us via in-app purchases during the last 12 months (or €0 if you have made no purchases).

### 9.2 Exclusions

We cannot be held liable for:

- Losses or damages caused by **fraudulent behavior** of other users.
- **Malfunctions** of mobile networks, internet providers, App Store / Play Store.
- **Data loss** due to a major Firebase incident or your own account deletion.
- **Incorrect match data** provided by our partner football-data.org.

---

## 10. Termination

### 10.1 By you

You can at any time delete your account via Settings → Delete my account. Deletion is immediate and irreversible.

### 10.2 By us

We may terminate your access to the Application in case of:

- Serious violation of these ToS.
- Fraudulent or abusive behavior.
- Prolonged inactivity (account inactive > 24 months — email notice).
- Service cessation.

In case of termination for violation, no compensation is due.

---

## 11. Modifications to ToS

We may modify these ToS at any time. The "Last updated" date will be updated at the top of the document.

In case of substantial modification, you will be informed by in-app notification or email **30 days before** the changes take effect. Continued use of the Application after this date constitutes acceptance of the new ToS.

---

## 12. Applicable law and jurisdiction

These ToS are governed by **French law**.

Any dispute relating to the use of the Application shall, failing prior amicable resolution, be subject to the exclusive jurisdiction of the courts of the defendant's domicile (or the consumer's, in accordance with mandatory consumer protection law provisions).

---

## 13. Contact

For any questions about these ToS:

- **Email**: support@fcpronox.com
- **Address**: Mehdi Bouhaouala, Courbevoie, France

---

*PronoGo is published by **Mehdi Bouhaouala**, natural person (private individual), resident in Courbevoie, France. The application is made available free of charge and on a non-commercial basis. These ToS will be updated if the publisher's status changes (sole proprietor, company) or if paid features (IAP, advertising) are activated.*
